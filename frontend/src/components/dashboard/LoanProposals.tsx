...
export default function LoanProposals() {
    ...
    async function fetchProposals() {
      try {
        ...
        if (res.ok) {
          ...
          // Fallback dev stub data for testing multi-sig proposal thresholds
          setProposals([
            {
              id: 'PROP-042',
  ...
  const handleCastVote = async (id: string) => {
    setVotingId(id);
    try {
      // Mocking the multi-sig approval endpoint matching Raymond's routing specs
      ...
      if (res.ok || true) { // Simulating state increment smoothly for dev testing

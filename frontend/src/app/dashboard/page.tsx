...
export default function DashboardTestPage() {
  ...
  useEffect(() => {
    if (connected && wallet) {
      // Canonical bech32 address (Module 1) — must match what registration
      // stored, or member/loan lookups silently miss.
      resolveWalletAddress(wallet).then(addr => {
        setAddress(addr);
        // Member profile + neighbor search are reads — not signature-gated
        // (AUTH_CONTRACT), so a plain fetch with the bech32 address is enough.
        fetch('/api/members', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },

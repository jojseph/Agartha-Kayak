...
/**
 * PATCH /api/loans/treasury/manage
 * Elder or Owner manually updates a treasury loan lifecycle status.
 *
 * Allowed transitions (elder-driven, off-chain governed):
 *   approved/active → overdue         (elder flags missed payment)
 *   approved/active/overdue → defaulted (elder marks default after no recovery)
 *   overdue → approved/active          (borrower caught up — elder reinstates)
 *
 * Rules:
 * - Only elders/owners can call this
 * - The elder CANNOT manage their own treasury loan
 *
 * Body: { loanId, newStatus, elderAddress, reason? }
 */
export async function PATCH(request: Request) {
    ...
    try {
        ...
        // Self-protection: elder cannot manage their own loan
        ...
        const VALID_TRANSITIONS: Record<string, string[]> = {
            'overdue':   ['approved', 'active'],         // flag missed payment
            'defaulted': ['approved', 'active', 'overdue'], // mark as defaulted
            'approved':  ['overdue'],                      // reinstate from overdue (caught up)
        };
        ...
        // Enqueue on-chain receipt
        if (elder.community_id) {
            const recordType = newStatus === 'defaulted' ? 'loan_defaulted'
                : newStatus === 'overdue' ? 'loan_approved'  // reuse existing type; summary clarifies
                : 'loan_approved';
